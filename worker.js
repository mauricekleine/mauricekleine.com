...
function parseEssayIndex(markdown, origin) {
...
function essayOpening(markdown) {
...
async function listEssays(env, origin) {
  const md = await fetchText(env, origin, "/essays.md");
  return md ? parseEssayIndex(md, origin) : [];
}

async function getEssay(env, origin, slug, full) {
  if (!slug) return null;
...
async function makeSubscribeToken(secret, email) {
...
async function verifySubscribeToken(secret, token) {
...
async function resendConfirmContact(env, email) {
...
function subscribePage({ title, description, heading, message, back }) {
  return `<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
...
function subscribeErrorResponse(request, status, message) {
...
async function handleSubscribe(request, env, url) {
  const missing = ["RESEND_API_KEY", "TURNSTILE_SECRET", "SUBSCRIBE_SECRET"].filter((k) => !env[k]);
  if (missing.length) {
...
async function handleSubscribeConfirm(request, env, url) {
  const missing = ["RESEND_API_KEY", "RESEND_SEGMENT_ID", "SUBSCRIBE_SECRET"].filter((k) => !env[k]);
  if (missing.length) {
