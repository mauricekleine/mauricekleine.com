...
Bottom of `essays.html` and every `essays/<slug>.html` (before the older/newer nav) carries an identical email signup form: one email input, a Cloudflare Turnstile widget (site key is the placeholder `TURNSTILE_SITE_KEY_PLACEHOLDER`, swap it in one pass), a submit button, and one line of fine print. `subscribe.js` progressively enhances the form (fetch + inline result); without JS it posts to `/subscribe` and the worker replies with an html page. Double opt-in: `POST /subscribe` validates the email, checks Turnstile, upserts an unsubscribed Resend contact, and emails a signed confirmation link (7-day expiry, HMAC-SHA256 via WebCrypto); `GET /subscribe/confirm?t=...` verifies it, flips the contact to subscribed, and adds it to the Resend segment. Needs `RESEND_API_KEY`, `RESEND_SEGMENT_ID`, `TURNSTILE_SECRET`, `SUBSCRIBE_SECRET` as worker secrets. `tools/import-x-article.py` includes the same form on every new essay page; `tools/link-essays.py` only rewrites the essay-nav markers, not the form.
