...
- Contains a sentence that is true but changes nothing for the reader.
- Needs a second read to parse.
- Calls the same thing by two names.
...
- Reaches for a metaphor or flourish where the literal phrase would do ("earns its keep" instead of "still matters").
- Ends with "Agree?" or a hashtag stack.
- Turns a long article into a stack of one-line posts.
- Gives every section the same setup, three-item list, lesson and slogan.
- Uses generic bridges such as "here's where it gets interesting" or "the point is".
- Resolves a real contradiction into a tidy founder lesson.
- Could run under another founder's name without changing a word.
